    % gamma_simulator.m
function [spiketimes_i, spiketimes_e, avnfreq] = gamma_simulator(W, params, Iapp, ...
    num_e, num_i, g_hat_ee, g_hat_ei, g_hat_ie, g_hat_ii, tau_d_e_value, tau_d_i_value)   
    ...
    inhib_idx = sort(randperm(num_e+num_i, num_i));
    excit_idx = 1:(num_e+num_i); excit_idx(inhib_idx) = [];
    
    W_ee = W(excit_idx, excit_idx);
    W_ei = W(excit_idx, inhib_idx);
    W_ie = W(inhib_idx, excit_idx);
    W_ii = W(inhib_idx, inhib_idx);
    ...
    g_ee=g_hat_ee*W_ee.*(sign(p_ee-rand(num_e,num_e))+1)/2/(p_ee*num_e);
    g_ei=g_hat_ei*W_ei.*(sign(p_ei-rand(num_e,num_i))+1)/2/(p_ei*num_e);
    g_ie=g_hat_ie*W_ie.*(sign(p_ie-rand(num_i,num_e))+1)/2/(p_ie*num_i);
    g_ii=g_hat_ii*W_ii.*(sign(p_ii-rand(num_i,num_i))+1)/2/(p_ii*num_i);
    ...
    for k=1:m_steps
        ...
        v_e_inc=0.1*(-67-v_e)+80*n_e.^4.*(-100-v_e)+100*m_e.^3.*h_e.*(50-v_e) ...
                   ...
                   +I_e(t_old)*heaviside(t_pulse-t_old)+g_stoch_e*s_stoch_e.*(v_rev_e-v_e);
        ...
        v_i_inc=0.1*(-65-v_i)+9*n_i.^4.*(-90-v_i)+35*m_i.^3.*h_i.*(55-v_i) ...
                   ...
                   +I_i(t_old)*heaviside(t_pulse-t_old)+g_stoch_i*s_stoch_i.*(v_rev_e-v_i);
        ...
        v_e_inc=0.1*(-67-v_e_tmp)+80*n_e_tmp.^4.*(-100-v_e_tmp)+100*m_e_tmp.^3.*h_e_tmp.*(50-v_e_tmp) ...
                   ...
                   +I_e(t_mid)*heaviside(t_pulse-t_mid)+g_stoch_e*s_stoch_e.*(v_rev_e-v_e_tmp);
        ...
        v_i_inc=0.1*(-65-v_i_tmp)+9*n_i_tmp.^4.*(-90-v_i_tmp)+35*m_i_tmp.^3.*h_i_tmp.*(55-v_i_tmp) ...
                   ...
                   +I_i(t_mid)*heaviside(t_pulse-t_mid)+g_stoch_i*s_stoch_i.*(v_rev_e-v_i_tmp);
    ...
    nisis=nisis(nisis>1.0);
    avnfreq=1/(mean(nisis)/t_final);
